...
local effect3 = CONST_ME_BITE
local Range = 3

local function manaLeach(cid, pos)
	local player = Player(cid)	
	local hit_min = ((player:getLevel() / 5) + (player:getMagicLevel()*1) +6)/2
	local hit_max = ((player:getLevel() / 5) + (player:getMagicLevel()*2.6) +16)/2	
...
function onGetFormulaValues(player, level, magicLevel)
	local min = ((level / 5) + (magicLevel * 1) + 6)*1.5
	local max = ((level / 5) + (magicLevel * 2.6) + 16)*1.5
	return -min, -max
end
...
rune:group("attack")
rune:name("special life leach holy rune")
rune:runeId(40425)
