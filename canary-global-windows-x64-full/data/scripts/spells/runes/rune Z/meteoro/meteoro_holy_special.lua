...
function onGetFormulaValues1(player, level, magicLevel)
	local min = ((4*level / 5) + (magicLevel * 1.2) + 5)*1.5
	local max = ((4*level / 5) + (magicLevel * 2.85) + 16)*1.5
	return -min, -max
end

function onGetFormulaValues2(player, level, magicLevel)
	local min = ((2*level / 5) + (magicLevel * 0.8) + 5)*1.5
	local max = ((2*level / 5) + (magicLevel * 1.2) + 16)*1.5
	return -min, -max
end

function onGetFormulaValues3(player, level, magicLevel)
	local min = ((1*level / 5) + (magicLevel * 0.4) + 5)*1.5
	local max = ((1*level / 5) + (magicLevel * 0.8) + 16)*1.5
	return -min, -max
end
...
rune:group("attack")
rune:name("special meteoro holy rune")
rune:runeId(40436)
