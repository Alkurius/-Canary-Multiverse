...
function offlineTraining.onLogin(player)
	...
	--Add 20% of level for 12H
	local lvlplayer = player:getLevel()
	local expStage = getRateFromTable(experienceStages, player:getLevel(), configManager.getNumber(configKeys.RATE_EXP))
	local addExp = math.floor(trainingTime*((0.0002313*lvlplayer*lvlplayer)-(0.0002313*lvlplayer)+0.0005))
	local totalexp = addExp*expStage
	player:addExperience(totalexp, false)
	player:sendTextMessage(MESSAGE_OFFLINE_TRAINING, "During your absence you increased your experience in " .. totalexp .. " points.")
